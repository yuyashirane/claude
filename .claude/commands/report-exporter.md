# レポート出力スキル (report-exporter)

他のコマンド（accounting-audit、consumption-tax-checker、finance-analyzer等）の実行結果を、Excel(.xlsx)ファイルまたはGoogleスプレッドシートにプロフェッショナルなレポートとして出力する。

---

## 起動条件

以下のいずれかに該当する場合にこのコマンドを使用する:
- 他のコマンドの実行後に「レポート出力」「Excelに出力」「スプレッドシートに出力」「ファイルに出力」と指示された場合
- `/report-exporter` として直接呼び出された場合
- 実行結果を「保存」「エクスポート」「共有用に」と指示された場合
...
例:
```
reports/あしたの会計事務所税理士法人_勘定科目チェック_2024-10-01_2025-12-31_2026-03-20_2026-03-20T1037.xlsx
```

- 会社名が不明な場合は会社IDを使用（例: `474381`）
- スキル名はコマンドの種類に応じて自動設定:
  - accounting-audit → `勘定科目チェック`
  - consumption-tax-checker → `消費税区分チェック`
  - finance-analyzer → `財務分析`
  - overseas-service-tax-checker → `海外サービス消費税チェック`
  - その他 → コマンド名をそのまま使用
- 実行時刻は `HHmm` 形式（24時間制）
...
### consumption-tax-checker（消費税区分チェック）の場合
...
| レベル | アイコン | セル背景 | テキスト色 |
|--------|---------|---------|-----------|
| 要対応（Critical） | 🔴 | RED_BG | RED_TEXT |
| 要確認（Warning） | 🟡 | YELLOW_BG | YELLOW_TEXT |
| 良好（OK） | 🟢 | GREEN_BG | GREEN_TEXT |
| 参考情報（Info） | 🔵 | BLUE_INFO_BG | BLUE_INFO_TEXT |
...
1. **会話コンテキストからデータを収集**
   - 直前のコマンド実行結果（チャット内のテキスト出力）を構造化データとして解析
   - 元データファイル（CSVやfreee APIレスポンス等）があれば参照

2. **reportsディレクトリの確認・作成**
   ```bash
   mkdir -p reports
   ```
...
def generate_filename(company_name, skill_name, period_start, period_end):
    ...
    return f"reports/{company_name}_{skill_name}_{period_start}_{period_end}_{date_str}_{time_str}.xlsx"
```
...
1. ファイルパスをユーザーに表示する
2. シート数・主要データ件数を報告する
3. 出力先がGoogleスプレッドシートの場合はURLを表示する

---

## 注意事項

- 元のコマンド結果に含まれる全ての指摘事項・データを漏れなく出力すること
- 数値の桁区切り、パーセント表示などの書式を必ず適用すること
- 日本語フォント（Yu Gothic）を使用し、文字化けを防ぐこと
- セル幅は内容に応じて適切に設定すること（狭すぎて内容が見切れないように）
