...
- M-2：§4.6.2 改修後コードの args.verbose 不在対処（β 採用、`sink = io.StringIO()` 固定）
- M-5：§4.6.3 重要事項 6 に args.verbose 不在の前提を追加
- M-7：§5.0 事前確認を環境変数 α 前提から B-4-0-2 でのパス変更完了済前提に全面書き換え
- M-8：§5.5.2 例示パスを `tests/e2e/<id>/<period>/` 形式に訂正
- A-1：§5.5 に新規 5.5.3（wallet_txn_id 形式変更観察 + 事例 17 注釈）を追加
- A-2：§5.5.1 末尾と新規 §5.5.4（ID 衝突可能性観察）に観察不能指針を追加
- N-1 / N-2：§1.5 制約 1 / 2 に B-4 観察結果の注釈を追加

詳細根拠：`V1-3-20_beta2_D_L1_B_completion_log.md`（472 行）

注：v2 で既に修正済の M-1（V1-3-10 211 tests）/ M-3（引数名）/ M-4（ファイル名）/ M-6（ctx.company_id 型差異論点消滅）は v3 でも維持（変更なし）。
...
> ※ B-4 検証では本制約に該当する観察ケース（manual_journals 由来の Finding）が発生しなかったため、実機での挙動確認は L1-C 以降の課題として保留。
...
> ※ B-4 検証では 3 社すべてで partner 重複が 0 件だったため、後勝ち問題は顕在化せず。L1-C 以降の対応優先度は低（要観察）。
...
6. **必須引数（5 つ）**：`build_check_context` のシグネチャ（freee_to_context.py L374〜L383）に従い、以下 5 つは必須：`deals_path` / `partners_path` / `account_items_path` / `company_info_path` / `taxes_codes_path`。**ファイル名は実装上の規約と完全一致**（`account_items_all.json` / `company_info.json` / `taxes_codes.json`）。**前提として、`args.verbose` は V1-3-20 に存在しないため、§4.6.2 では `sink = io.StringIO()` 固定とする**（事例 16：B1_continuation_prompt.md §1.1 で β 採用確定、`_build_parser` は無変更）。
...
### 5.0 B-4 着手前の必須事前確認（最重要）

#### 5.0.1 前提：B-4-0-2 でパス変更完了済
...
> ※ B-4 検証で「manual_journals 流入 + Finding 化対象あり」の社が 0 社だった場合、本観察項目は「条件不成立」として「観察不能」を記録する。これは検証の失敗ではなく、検証セットの特性による自然な結果として扱う（L1-B 完了ログ §3.5 参照）。

#### 5.5.2 partner_master の同名 partner 後勝ち観察

```powershell
# partners_data の partner 名が重複していないかを確認
python -c "import json; from collections import Counter; p = json.load(open('C:/Users/yuya_/claude/office/office-claude/tests/e2e/3525430/2025-12/partners_all.json', encoding='utf-8')); names = [item.get('name') or item.get('long_name') or '' for item in p.get('partners', p) if isinstance(item, dict)]; dup = [n for n, c in Counter(names).items() if c > 1 and n]; print('Duplicate partner names:', dup)"

# 期待: 通常は重複なし。重複が多数の場合は L1-C 以降で別タスク化
```

#### 5.5.3 wallet_txn_id 形式変更の観察（B-4 で実機確認）

L1-B 経路移行（`_normalize_deals` → `build_check_context` + adapter）に伴い、`wallet_txn_id` の形式が `f"{deal_id}-{detail_id}"` から `str(detail["id"])` 単独に変化した（V1-3-10 と統一）。

```powershell
# 3 社で groups[].findings[].wallet_txn_id のサンプルを目視確認
python -c "import json; d = json.load(open('out_3525430.json', encoding='utf-8')); print([f['wallet_txn_id'] for g in d['groups'] for f in g['findings']])"

# 期待: L1-A 形式（'<deal_id>-<detail_id>'）から L1-B 形式（'<detail_id>'）に変化していること
# 件数・分類・partner 等の構造は不変
```

> ※ wallet_txn_id 形式変更は L1-B 経路移行の不可避な構造的副作用であり、「事例 17：経路移行に伴う ID 形式変化」として 2-J 失敗パターン集に記録予定（L1-B 完了ログ §3.4 / §6.2 参照）。

#### 5.5.4 ID 衝突可能性の観察（manual_journal × deal の wallet_txn_id 衝突）

`transform_deal_to_rows` と `transform_journal_to_rows` のいずれも `wallet_txn_id = str(detail["id"])` を使用。両者は freee API の独立した ID 空間に属するため、理論上は同じ値が衝突する可能性がある。

```powershell
# Finding の wallet_txn_id 集合と raw["source"] 由来の deal/manual_journal 区分を突合
# 衝突発生時は B-4 完了報告に明記、修正対応は L1-C 以降で判断
```
