...
| クラスタ | 内容 | 主な変更ファイル |
|---|---|---|
| **0**(前提) | 通常課税仕入の `tax_code` 範囲を 3 社で現物確認 → 報告 → GO 取得 | （調査のみ、コード変更なし） |
| **A** | schema 拡張 + classify_transaction + β1 ロジック削除 | `schema.py`, `run.py` |
| **B** | _normalize_deals 改修 + CLI 出力変更 + テスト総入れ替え + 実データ検証 | `run.py`, `tests/` |
...
通常課税仕入の `tax_code` 範囲を、推測ではなく 3 社で現物確認して確定する。

β2-A クラスタ 0 で経過措置範囲（183〜230）は確定済。今回は通常課税仕入の範囲を確定する。

### §4.0.2 確認対象（3 社）

β2-A と同じ 3 社：

- 3525430（アントレッド、2025-12）
- 12243357（キャラクターアニメ、2025-07）
- 10794380（デイリーユニフォーム、2025-12）

474381（自社）は β2-D で fetch 予定のため、本フェーズでは確認しない。

### §4.0.3 確認事項

3 社の `taxes_codes.json` を `view` で開き、以下を確認：

#### 3.1 通常課税仕入を示す税区分の特定

- `name_ja` に「課対仕入」「課税仕入」を含み、**かつ**「控80」「控50」を含まない税区分の `code` 値を抽出
- 例：「課対仕入10%」「課対仕入8%(軽)」「課税仕入10%」等

#### 3.2 通常課税仕入の `tax_code` 範囲

- 抽出した code が連続範囲か、飛び地か
- 範囲の最小値・最大値
- 経過措置範囲（183〜230）と重複していないか
...
3 社の現物を踏まえて、`FULL_DEDUCTION_TAX_CODES` をどう定義すべきかを提案。

### §4.0.4 報告フォーマット

```md
## 通常課税仕入 tax_code 範囲 現物確認結果
...
各社で抽出した code 値（最大 20 件まで列挙、それ以上は範囲表記）：
- 3525430: [..., ..., ...]
- 12243357: [..., ..., ...]
- 10794380: [..., ..., ...]
...
### code の範囲（最小〜最大）
- 連続範囲か、飛び地か
- 経過措置範囲（183〜230）との重複: なし / あり

### 3 社間の一致
- 一致 / 差異あり、差異の具体内容

### 推奨 FULL_DEDUCTION_TAX_CODES 定義案

```python
FULL_DEDUCTION_TAX_CODES = frozenset(...)  # 具体定義
```
...
1. コード変更を行わない
2. 推測で範囲を固定しない
3. 範囲を独断で確定しない（推奨案として提示するのみ）
4. 4 社目（474381）の fetch をしない（β2-D 担当）
5. 想定外論点を独断で進めない（運用原則 12）

### §4.0.6 完了条件

- [ ] 3 社の `taxes_codes.json` を view で確認
- [ ] §4.0.4 報告フォーマットの全項目記載
- [ ] 推奨 `FULL_DEDUCTION_TAX_CODES` 定義案を提示
- [ ] 想定外論点があれば即報告

**クラスタ 0 完了 + GO 取得まで、クラスタ A の実装には着手しない。**
...
FULL_DEDUCTION_TAX_CODES = frozenset(...)  # ← クラスタ 0 で確定後に定義
"""通常課税仕入コード範囲（β2-B クラスタ 0 で確定）。"""
```
...
- `classification_counts` の合計が deals 件数と一致

**重要**：件数の妥当性は β2-D で評価する。β2-B では **「正しく動いているか」のみ** 確認。0 件でも多すぎてもエラーにしない。
...
## §13. では、クラスタ 0 から開始してください

### 最初のアクション

1. β2-A メモ（`V1-3-20_beta2_A_data_structure_policy.md`）を view で確認
2. クラスタ 0：3 社の `taxes_codes.json` を view で確認
3. §4.0.4 の報告フォーマットで報告

**クラスタ 0 の GO が出るまで、クラスタ A の実装には着手しないでください。**
