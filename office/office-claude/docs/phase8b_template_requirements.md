...
Excel UI で Named Style を直接作るのは煩雑なので、以下のスクリプトで一括追加する。Claude Code が提供する確定版スクリプトは、悠皓さんの最終確認後に `scripts/add_named_styles.py` として生成する。

```python
# scripts/add_named_styles.py（参考実装）
...
Claude Code が悠皓さん作業後に `scripts/verify_phase8b_template.py` を生成・実行する（検証後は削除）。
